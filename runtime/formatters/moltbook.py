...
def send(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    cmd = [sys.executable, str(SCRIPT), "--submolt", submolt, "--content", content]
    if title:
    ...
    # script prints all errors to stdout; fall back when stderr empty
    diag = stderr or stdout
    low = diag.lower()
    if result.returncode != 0:
        if "401" in diag or "unauthorized" in low or "invalid api key" in low:
            raise AuthFailure(f"moltbook auth: {diag[:500]}")
        if "429" in diag or "rate limit" in low or "rate limited" in low:
            raise TransientError(f"moltbook rate-limited: {diag[:500]}")
        raise TransientError(f"moltbook failed rc={result.returncode}: {diag[:500]}")
