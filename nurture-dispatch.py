...
# Unified fail-closed send gate (2026-07-13). If the import breaks, we must
# refuse to send rather than bypass — this path used to skip every gate.
try:
    from runtime.kill_switches import is_send_allowed
except Exception as _gate_exc:
    _GATE_IMPORT_ERROR = _gate_exc

    def is_send_allowed(email, **kwargs):
        return False, f"gate_unavailable:{type(_GATE_IMPORT_ERROR).__name__}:{_GATE_IMPORT_ERROR}"
...
def send_email(to_email, subject, html_body, dry_run=False):
    ...
    # Unified gate: master kill + RICK_EMAIL_SEND_LIVE + merged suppression.
    # cold=False — nurture cadence is controlled by EMAIL_DELAYS_HOURS.
    allowed, gate_reason = is_send_allowed(to_email, cold=False)
    if not allowed:
        log.error("SEND_BLOCKED reason=%s to=%s", gate_reason, to_email)
        return False
    payload = {
        "from": FROM_EMAIL,
        "to": [to_email],
...
def load_suppression_set():
    """Hard-bounced + manually suppressed addresses — never send to these (2026-07-13)."""
    suppressed = set()
    ops = Path.home() / "rick-vault" / "operations"
    # Merged suppression: legacy ops copy + canonical mailbox list + DNC list.
    for txt in (ops / "suppression.txt",
                Path.home() / "rick-vault" / "mailbox" / "suppression.txt",
                Path.home() / "rick-vault" / "control" / "dnc-list.txt"):
        if txt.exists():
            suppressed.update(l.split("#", 1)[0].strip().lower() for l in txt.read_text().splitlines()
                              if l.split("#", 1)[0].strip())
    bounces = ops / "email-bounces.jsonl"
    if bounces.exists():
        for line in bounces.read_text().splitlines():
            try:
                entry = json.loads(line)
            except json.JSONDecodeError:
                continue
            if entry.get("event") == "bounced":
                addr = (entry.get("email") or entry.get("to") or entry.get("recipient") or "").strip().lower()
                if addr:
                    suppressed.add(addr)
    return suppressed
...
def process_contacts(state, dry_run=False):
    ...
    for email, contact in contacts.items():
        ...
        if normalized in suppressed or ("@" in normalized and "@" + normalized.rsplit("@", 1)[-1] in suppressed):
            contact["status"] = "suppressed"
            log.info("Suppressed (bounce/manual/DNC list): %s", email)
            continue
